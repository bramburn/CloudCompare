...
private slots:
    ...
    void testMakeNormalsHorizontal_data()
    {
        ...
        QTest::newRow("already_horizontal") <<
            std::vector<std::array<double,3>>{{1, 0, 0}} <<
            std::vector<std::array<double,3>>{{1, 0, 0}};
        ...
        // After MakeNormalsHorizontal: Z=0, renormalize → (1/√2, 1/√2, 0) ≈ (0.707, 0.707, 0)
        {
            ...
            QTest::newRow("tilted_111") <<
                std::vector<std::array<double,3>>{{1, 1, 1}, {0, 0, 1}} <<
                std::vector<std::array<double,3>>{{invSqrt2, invSqrt2, 0.0}, {0.0, 0.0, 0.0}};
        }
        ...
        // horizontal: (-1/√14, 2/√14, 0) ≈ (-0.267, 0.535, 0)
        {
            ...
            QTest::newRow("negative_tilted") <<
                std::vector<std::array<double,3>>{{-1, 2, -3}} <<
                std::vector<std::array<double,3>>{{-invSqrt14, 2*invSqrt14, 0.0}};
        }
    }
