...
/**
 * @file qAnimationDlg.cpp
 *
 * @brief Animation dialog implementation
 *
 * Main dialog for the CloudCompare animation plugin:
 * - **Keyframe management**: add, remove, reorder viewpoints
 * - **Interpolation**: linear, spherical, cubic between keyframes
 * - **Playback**: play, pause, speed control, loop
 * - **Output**: export to video (FFMPEG) or image sequence
 *
 * ## Animation System
 *
 * Keyframes store: camera position, target, up vector, FOV,
 * and scalar field display parameters. ViewInterpolate
 * generates intermediate frames between keyframes.
 *
 * @see qAnimation.h
 */
