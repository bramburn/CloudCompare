 ...
 * @file qBroomDlg.cpp
 *
 * @brief Broom tool dialog implementation
 *
 * The "broom" tool smooths and levels point clouds by
 * fitting a plane through the selected region and shifting
 * points vertically to align with the plane.
 *
 * ## Algorithm
 *
 * 1. User paints a region on the point cloud
 * 2. Fit a least-squares plane to the painted points
 * 3. Compute the plane equation: Z = aX + bY + c
 * 4. Shift each point in the painted region so its
 *    Z coordinate matches the fitted plane
 *
 * ## Use
 *
 * Used to flatten terrain scans or level scan faces
 * after registering scans with residual tilts.
 *
 * @see qBroomDlg.h
 */
#include "qBroomDlg.h"
