 ...
 * @file ccCompass.cpp
 *
 * @brief Compass plugin implementation
 *
 * Structural geology field mapping plugin for CloudCompare:
 * digitizes and analyses geological features in point clouds.
 *
 * ## Features
 *
 * - **Point clouds**: import and manage TLS/photogrammetric scans
 * - **Geo-objects**: digitize planes (bedding, foliation), lines (fold axes),
 *   pinch nodes, and thickness markers
 * - **Topology**: build a geological topology graph (cross-cutting relations)
 * - **Measurements**: export strike/dip, plunge/bearing, apparent thickness
 * - **Export**: DXF, CSV, shapefile, Compass XML project
 *
 * ## Data Model
 *
 * - ccSNECloud: points with scan event annotations
 * - ccGeoObject: geological feature (plane, line, node)
 * - ccTopologyRelation: relationship between two geo-objects
 *
 * @extends QObject
