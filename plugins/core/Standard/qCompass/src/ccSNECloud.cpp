 ...
 * @file ccSNECloud.cpp
 *
 * @brief Compass SNE (Scan Near Event) cloud implementation
 *
 * A point cloud with Scan Near Event annotations:
 * each point carries metadata about scan events
 * (which scan it was captured by, return number, etc.).
 *
 * ## SNE Model
 *
 * TLS scans capture overlapping point clouds. The SNE
 * model annotates each point with which scan station
 * acquired it, enabling per-scan analysis.
 *
 * @see ccSNECloud.h
 */
