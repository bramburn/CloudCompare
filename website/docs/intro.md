...
- **A self-contained Windows build artifact**
  ...
  box, runs.
- **A slim GitHub Actions matrix** (Windows MSVC only) that mirrors
  the local 18-plugin set. **macOS and Linux were dropped from the
  matrix on 2026-08-24**; see
...
- The exact `cmake` invocation used to build the fork on Windows (the
  sole CI platform). macOS and Linux are community-supported, not
  CI-validated.
