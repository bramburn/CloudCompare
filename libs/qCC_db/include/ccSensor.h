 ...
 *
 * Base class for sensors. New framework stores positions separately
 * (from GPS/IMU), with sensors holding only intrinsic parameters
 * and a rigid transformation to the position reference frame.
 */
class QCC_DB_LIB_API ccSensor : public ccHObject
{
  public:
	 ...
	 * @return Sensor type (should be reimplemented by subclasses)
	 */
