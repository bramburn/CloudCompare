...
- Wired Sentry (one DSN, one release per session)
...
4. **Document the pattern in `docs/patterns.md`** — explain why the template looks the way it does.
...
- [ ] Wire Sentry with the repo DSN (in release builds, gated by `SENTRY_DSN` env)
...
- Why templates exist: see `../docs/patterns.md` (TODO)
