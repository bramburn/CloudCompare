//! ICP variant 2 — kiddo KD-tree — CLI benchmark.

use icp_v2_kiddo::{build_kdtree, icp_iterate, default_params, nearest};
...
fn main() {
    ...
    eprintln!("kiddo KD-tree ICP: {} points", n);

    // Phase 1: build the KD-tree
    let n_model = model.len() / 3;
    let model_arr: Vec<[f64; 3]> = (0..n_model)
        .map(|i| [model[i*3] as f64, model[i*3+1] as f64, model[i*3+2] as f64])
        .collect();
    let build_start = std::time::Instant::now();
    let tree = build_kdtree(&model_arr);
    let build_elapsed = build_start.elapsed();
    println!("build_time   = {:.3} s", build_elapsed.as_secs_f64());

    // Phase 2: 1000 NN queries
    let query_points: Vec<[f64; 3]> = (0..1000)
        .map(|i| {
            let idx = (i * 7) % n_model;
            [model[idx*3] as f64 + 0.1, model[idx*3+1] as f64, model[idx*3+2] as f64]
        })
        .collect();
    let query_start = std::time::Instant::now();
    let mut total_nn = 0;
    for q in &query_points {
        let _ = nearest(&tree, q);
        total_nn += 1;
    }
    ...
    // Phase 3: full ICP via cc-rust (corrected algorithm, brute-force NN)
